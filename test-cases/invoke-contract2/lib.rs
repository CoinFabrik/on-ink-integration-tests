...
mod invoke_contract {
    ...
    mod tests {
        use ink::{env::instantiate_contract, primitives::AccountId};
        use super::*;

        #[ink::test]
        fn invoke_contract_works() {
            let code_hash = ink::env::simulate_code_upload::<ink::env::DefaultEnvironment, ContractToCallRef>();
            ...
            let address2: &[u8; 32] = address.as_mut();
            let address = address2.clone();
            println!("address: {:?}", address);

            let contract = InvokeContract::new();
            let profit = contract.invoke_call(address);
