...
mod ecdsa_recover {
    ...
    mod tests {
        ...
        fn ecdsa_recover_works() {
            ...
            // When
            let ecdsa_key = contract.ecdsa_recover(SIGNATURE, MESSAGE_HASH);

            // Then
            assert_eq!(ecdsa_key, EXPECTED_COMPRESSED_PUBLIC_KEY);
        }
    }
    ...
    mod e2e_tests {
        ...
        async fn ecdsa_recover_works(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            ...
            let ecdsa_recover_call = build_message::<ECDSARecoverRef>(contract_acc_id)
                .call(|contract| contract.ecdsa_recover(SIGNATURE, MESSAGE_HASH));
